...
export async function seedQuizQuestions() {
  const row = await dbGet('SELECT COUNT(*)::int AS count FROM quiz_questions', []);
  if (row && row.count > 0) {
    console.log(`[SEED] Quiz questions already seeded (${row.count} rows). Skipping.`);
    return row.count;
  }
  ...
  console.log(`[SEED] Seeded ${rows.length} quiz questions from question bank.`);
  return rows.length;
}
